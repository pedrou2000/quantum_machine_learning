...
class MMD_GAN:
    def __init__(self, z_dim=1, gen_hidden_units=[7, 13, 7], critic_hidden_units=[11, 29, 11],
        ...
        n=100, mean=0, variance=1, results_path="results/1-initial_tests/gan/1d/mmd/", print_frequency=10):
    ...
    def sample_noise(self):
        return tf.random.normal((self.batch_size, self.z_dim))  # Sample one latent dimension for each batch
    ...
    def create_generator(self):
        ...
        model.add(ELU())
        model.add(Dense(self.gen_hidden_units[3]))
        return model

    def create_critic(self):
        model = Sequential()
        model.add(Dense(self.critic_hidden_units[0], input_dim=self.z_dim))
        model.add(ELU())
        model.add(Dense(self.critic_hidden_units[1]))
        model.add(ELU())
        model.add(Dense(self.critic_hidden_units[2]))
        model.add(ELU())
        model.add(Dense(self.critic_hidden_units[3]))
        return model
        ...
        for epoch in range(self.epochs):
            ...
            for _ in range(self.update_ratio_critic):
                ...
                with tf.GradientTape() as tape:
                    x_fake1 = tf.reshape(tf.repeat(self.generator(z1), self.n, axis=-1), (self.batch_size, self.n, 1))  # Reshape generator output to match the batch shape
                    x_fake2 = tf.reshape(tf.repeat(self.generator(z2), self.n, axis=-1), (self.batch_size, self.n, 1))  # Reshape generator output to match the batch shape
...
if __name__ == "__main__":
    ...
    print_frequency = 10
    z_dim = 1
    gen_hidden_units = [7, 13, 7, 1]
    critic_hidden_units = [11, 29, 11, 1]
    lrs = [0.01]
    critic_lr = 1e-3
    batch_size = 64
    ...
    means = [10]
    variances = [1]
    results_path = "results/1-initial_tests/gan/1d/mmd/pointwise/"
