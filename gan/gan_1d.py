...
class GAN:
    ...
    def target_distribution(self):
        if self.target_normal:
            return np.random.normal(self.target_mean, self.target_sd, self.batch_size)
        else:
            return np.random.uniform(self.target_mean, self.target_sd, self.batch_size)
    ...
    def plot_results(self, folder_path):
        ...
        plt.hist(self.target_distribution(), bins=50, alpha=0.6, label='Real Data')
        plt.legend()
